...
class MaskedLoss(K.losses.LossFunctionWrapper):
    def __init__(self, loss_function, masking_value, name='masked_loss',
        ...
        def _masked_loss(y_true, y_pred):
            ...
            lengths = tf.math.add(lengths, 1,
                                  name="masked_loss__sum_to_include_first_padding")  # to also include the first padding character
            mask = tf.sequence_mask(
                lengths=lengths,
                maxlen=y_pred.shape[-2],
                # pre-last dimension = padding length; last dimension = one-hot-encoded alphabet
                dtype=tf.float32,
            ...
            # print(summed_losses)
            average_losses = tf.math.divide_no_nan(summed_losses, lengths,
